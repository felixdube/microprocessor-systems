...
void Thread_Acc (void const *argument) {
		while(1){
			...
			//0 for the timeout value so that it never starts before the flag
				...
				ReadAcc();
				printf("%f\n",accValue[0]);
				osSignalClear(tid_Thread_Acc,0x01);
			}
	}
/*----------------------------------------------------------------------------
 *      Initialize the GPIO associated with the Acc
 *---------------------------------------------------------------------------*/
	/**
...
/*----------------------------------------------------------------------------
 *      Functions for Accelerometer thread
 *---------------------------------------------------------------------------*/
